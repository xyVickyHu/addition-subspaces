"""Substep 2 — selected-head extraction from the coefficient matrix.

Artifact (kind ``selected_heads``,
``<matrix-node>/selected-<method>-v<V>/selected_heads.json``):
...
- ordered selected heads with their (signed) coefficients;
...
ENGINE_VERSION = 2
IMPL = {"module": "subspaces.step1.selected", "algorithm_version": ENGINE_VERSION}
...
def impl_for(selected_cfg) -> dict:
    """Implementation identity for one selected config: shared-engine
    version plus the method's own (name, version)."""
    return {
        **IMPL,
        "method": {"name": selected_cfg.method, "version": selected_cfg.method_version},
    }


def selected_identity_config(selected_cfg) -> dict:
    """The selected config slice that enters artifact identity: method,
    method_version, and ONLY the parameters registered for that (name,
    version). A future parameter FIELD added to SelectedConfig therefore
    cannot fork existing methods' identities — it only enters the identity
    of methods whose schema names it."""
    schema = METHOD_PARAM_SCHEMAS[(selected_cfg.method, selected_cfg.method_version)]
    identity = {
        "method": selected_cfg.method,
        "method_version": selected_cfg.method_version,
    }
    for param in sorted(schema):
        identity[param] = getattr(selected_cfg, param)
    return identity
...
def extract_selected(
    ...
    """Select selected heads; returns the ``selected_heads`` manifest.
    ...
    method = cfg.selected.method
    key = (method, cfg.selected.method_version)
    if key not in METHOD_PARAM_SCHEMAS:  # config validation makes this unreachable
        raise ConfigError(f"unknown selected-set method {key!r}")
    gap_info = None
    if key == ("elbow", 1):
        threshold = auto_threshold(matrix, method="elbow")
    elif key == ("fraction", 1):
        threshold = auto_threshold(
            matrix, method="fraction", fraction=cfg.selected.fraction
        )
    elif key == ("fixed", 1):
        threshold = auto_threshold(
            matrix, method="fixed", fixed=cfg.selected.fixed_threshold
        )
    elif key == ("largest_gap", 1):
        threshold, gap_info = largest_gap_threshold(matrix)
    else:
        raise ArtifactError(
            f"selected-set method {key!r} is registered but not dispatched — "
            "update extract_selected"
        )
    ...
    return make_manifest(
        ...
        config=envelope.get(
            "config",
            {"selected": selected_identity_config(cfg.selected)},
        ),
