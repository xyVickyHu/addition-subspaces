...
that main-head scans descend from the supplied significant artifact."""
...
def _significant_manifest(fake_paths, matrix_ref, ref_path, heads):
    cfg = _cfg(fake_paths)
    return make_manifest(
        kind="significant_heads",
        schema_version=significant_mod.SIGNIFICANT_SCHEMA_VERSION,
        paths=fake_paths,
        config={
...
def test_stale_significant_artifact_refused(fake_paths):
    """A significant_heads.json recorded against a DIFFERENT matrix identity
    ...
    write_json_atomic(sig_node / "significant_heads.json", stale)

    with pytest.raises(ArtifactError, match="refusing to reuse"):
        stage_significant(cfg, fake_paths, matrix_node, matrix_ref)


def test_matching_significant_artifact_reused_without_recompute(
    ...
    sig_node = tree.sig_node_dir(matrix_node, cfg.significant)
    write_json_atomic(sig_node / "significant_heads.json", valid)

    def _must_not_run(*_a, **_k):
        raise AssertionError("select_significant must not run on valid reuse")

    monkeypatch.setattr(significant_mod, "select_significant", _must_not_run)
    reused, reused_node = stage_significant(cfg, fake_paths, matrix_node, matrix_ref)
    assert reused["heads"] == [[15, 2, 0.5], [13, 6, 0.3]]
    assert reused_node == sig_node


@pytest.fixture()
def lineage_chain(fake_paths, tmp_path):
    """significant -> head_scan -> main_heads, with correct reference chain."""
    significant = make_manifest(
        kind="significant_heads",
        schema_version=1,
        paths=fake_paths,
        payload={
            "impl": significant_mod.IMPL,
...
def test_compose_heads_refuses_foreign_significant(fake_paths, tmp_path, lineage_chain):
    """The scan descends from significant A; composing with significant B
...
def test_compose_heads_refuses_unlineaged_main(fake_paths, tmp_path, lineage_chain):
    significant_path, _ = lineage_chain
