"""Selected-head substep: Lane-A equivalence on the canonical matrix
...
from subspaces.artifacts import ArtifactError, modernize, write_json_atomic
...
from subspaces.step1.pipeline import ensure_run, stage_matrix, stage_selected
from subspaces.step1.selected import extract_selected
...
def test_canonical_matrix_exact_33_equivalence():
    ...
    shipped repro scan's selected set EXACTLY: 33 heads, same order,
    ...
    manifest = extract_selected(matrix_ref, cfg, paths)
    ...
    assert got == fixture["selected_heads"]
...
def test_toy_matrix_elbow_matches_legacy(fake_paths):
    ...
    manifest = extract_selected(matrix_ref, cfg, fake_paths)
...
def test_fixed_and_fraction_methods(fake_paths):
    ...
    cfg.selected.method = "fixed"
    cfg.selected.fixed_threshold = 0.5
    manifest = extract_selected(matrix_ref, cfg, fake_paths)
    ...
    cfg.selected.method = "fraction"
    cfg.selected.fixed_threshold = None
    cfg.selected.fraction = 0.99
    manifest = extract_selected(matrix_ref, cfg, fake_paths)
...
def test_toy_matrix_largest_gap(fake_paths):
    ...
    upper edge)."""
    cfg = _toy_cfg(fake_paths)
    cfg.selected.method = "largest_gap"
    matrix_ref = resolve_reuse_matrix(cfg, fake_paths)
    manifest = extract_selected(matrix_ref, cfg, fake_paths)
    heads = [(layer_idx, head_idx) for layer_idx, head_idx, _ in manifest["heads"]]
...
def test_largest_gap_threshold_edge_cases():
    ...
    from subspaces.step1.selected import largest_gap_threshold
...
def test_canonical_matrix_largest_gap_anchor():
    ...
    got = {(layer_idx, head_idx) for layer_idx, head_idx, _ in manifest["heads"]}
    expected = {
        (layer_idx, head_idx) for layer_idx, head_idx in fixture["selected_heads"][:28]
    }
    assert got == expected
...
def test_stage_selected_method_swap_forks_sibling_node_never_clobbers(fake_paths):
    """Under the lineage tree, changing selected.method no longer refuses
    in-place: method variants are SIBLING selected-* nodes under the matrix node.
    ...
    occupies that node's slot, stage_selected REFUSES instead of
    overwriting."""
    cfg = _toy_cfg(fake_paths)
    journal_dir, _, _ = ensure_run(cfg, fake_paths)
    matrix_ref, matrix_node = stage_matrix(cfg, fake_paths, journal_dir)
    elbow_manifest, elbow_node = stage_selected(
        cfg, fake_paths, matrix_node, matrix_ref
    )

    cfg.selected.method = "largest_gap"
    gap_node_expected = tree.selected_node_dir(matrix_node, cfg.selected)
    ...
    write_json_atomic(gap_node_expected / "selected_heads.json", elbow_manifest)
    ...
    (gap_node_expected / "selected_heads.json").unlink()
    gap_manifest, gap_node = stage_selected(cfg, fake_paths, matrix_node, matrix_ref)
    assert gap_node == gap_node_expected
    assert gap_manifest["threshold_method"] == "largest_gap"
    assert (elbow_node / "selected_heads.json").is_file()
    assert (gap_node / "selected_heads.json").is_file()


def test_selected_identity_config_is_method_scoped():
    ...
    SelectedConfig (always None for existing methods) can never fork
    their identities, because absent-from-schema fields never enter."""
    from subspaces.config import SelectedConfig
    from subspaces.step1.selected import selected_identity_config

    assert selected_identity_config(SelectedConfig()) == {
        "method": "elbow",
        "method_version": 1,
    }
    assert selected_identity_config(
        SelectedConfig(method="fraction", fraction=0.9)
    ...
    assert selected_identity_config(
        SelectedConfig(method="elbow", fixed_threshold=None, fraction=None)
    ) == selected_identity_config(SelectedConfig())


def test_selected_dirname_params_derive_from_registry():
    import re

    from subspaces.config import SelectedConfig
    from subspaces.step1.tree import selected_dirname

    assert selected_dirname(SelectedConfig()) == "selected-elbow-v1"  # no param hash
    name = selected_dirname(SelectedConfig(method="fraction", fraction=0.9))
    assert re.fullmatch(r"selected-fraction-v1-[0-9a-f]{6}", name)
...
def test_registered_but_undispatched_method_refuses(fake_paths, monkeypatch):
    ...
    monkeypatch.setitem(selected_mod.METHOD_PARAM_SCHEMAS, ("foo", 1), frozenset())
    cfg = _toy_cfg(fake_paths)
    cfg.selected.method = "foo"  # bypasses load-time validation on purpose
    matrix_ref = resolve_reuse_matrix(cfg, fake_paths)
    with pytest.raises(ArtifactError, match="registered but not dispatched"):
        selected_mod.extract_selected(matrix_ref, cfg, fake_paths)
...
def test_select_refuses_checkpoint_tamper(fake_paths):
    ...
    with pytest.raises(ArtifactError, match="checkpoint content mismatch"):
        extract_selected(matrix_ref, cfg, fake_paths)


def test_stage_selected_end_to_end_and_reuse(fake_paths, monkeypatch):
    cfg = _toy_cfg(fake_paths)
    journal_dir, _, _ = ensure_run(cfg, fake_paths)
    matrix_ref, matrix_node = stage_matrix(cfg, fake_paths, journal_dir)
    first, selected_node = stage_selected(cfg, fake_paths, matrix_node, matrix_ref)
    assert selected_node == tree.selected_node_dir(matrix_node, cfg.selected)
    assert (selected_node / "selected_heads.json").is_file()

    import subspaces.step1.selected as selected_mod
    ...
    monkeypatch.setattr(pipeline_mod.selected_mod, "extract_selected", _must_not_run)
    second, selected_node_again = stage_selected(
        cfg, fake_paths, matrix_node, matrix_ref
    )
    assert second == first
    assert selected_node_again == selected_node
